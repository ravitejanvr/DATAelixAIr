...
export default function BenchmarkV10Panel() {
  ...
  const runSingle = useCallback(async (mode: V10PipelineMode) => {
    ...
    setRunMode(mode);
    try {
      const result = await runV10Suite(mode, handleProgress, { executionMode: "production", parallelCases: 5 });
  ...
  const runComparison = useCallback(async () => {
    ...
    try {
      setRunMode("phase8");
      const p8 = await runV10Suite("phase8", handleProgress, { executionMode: "production", parallelCases: 5 });
      setPhase8Baseline(p8);

      setRunMode("phase9");
      const p9 = await runV10Suite("phase9", handleProgress, { executionMode: "production", parallelCases: 5 });
  ...
  const runThreeWayComparison = useCallback(async () => {
    ...
    try {
      ...
      setRunMode("phase10");
      const p10 = await runV10Suite("phase10", handleProgress, { executionMode: "production", parallelCases: 5 });
      setSuiteResult(p10);
