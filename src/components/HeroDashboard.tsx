...
const HeroDashboard = () => (
  ...
  >
    <div className="bg-card border border-border rounded-[20px] p-7 shadow-card relative overflow-hidden">
      ...
      {/* Metrics - real impact data */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { val: "30", unit: "%", label: "Less Doc Time" },
          { val: "£50K", unit: "+", label: "Savings/Year" },
          { val: "2x", unit: "", label: "Patient Time" },
        ].map((m) => (
    ...
    <div className="grid grid-cols-2 gap-3 mt-3">
      <div className="bg-card border border-border rounded-[14px] p-4 shadow-card">
        ...
        <p className="font-display text-lg font-bold text-primary mt-0.5">Underway</p>
      </div>
      <div className="bg-card border border-border rounded-[14px] p-4 shadow-card">
        <div className="w-9 h-9 rounded-[10px] teal-muted-bg flex items-center justify-center text-base mb-2.5">⚡</div>
        <p className="text-xs font-medium text-foreground">AI Transcription</p>
        <p className="font-display text-lg font-bold text-primary mt-0.5">Real-Time</p>
      </div>
    </div>
  </motion.div>
);
