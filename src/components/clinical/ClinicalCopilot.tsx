...
import { Brain, FlaskConical, Pill, Shield, CheckCircle, AlertTriangle, ChevronDown, ChevronRight, FileText } from "lucide-react";
          ...
          <Collapsible open={evidenceExpanded} onOpenChange={setEvidenceExpanded}>
            <CollapsibleTrigger asChild>
              <button className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg border border-border bg-card hover:bg-muted/50 transition-colors text-left">
                ...
                <Badge variant="outline" className="text-[10px]">{evidenceSources.length}</Badge>
                {evidenceExpanded ? (
                ...
                <div className="space-y-1.5">
                  {evidenceSources.map((src, i) => (
                    <div key={i} className="text-xs">
                      <p className="font-semibold text-foreground">{src.source}</p>
                      <p className="text-muted-foreground text-[11px]">{src.title}</p>
                      {src.url && (
                        <a
                          href={src.url}
                          target="_blank"
