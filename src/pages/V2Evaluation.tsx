...
export default function V2Evaluation() {
  ...
  const downloadReport = useCallback((suite: EvalSuiteResult, m: FullMetrics, v: ReturnType<typeof getSystemVerdict>) => {
    const lines: string[] = [];
    lines.push("# V2 Probabilistic Engine — Evaluation Report");
    lines.push(`\n**Date:** ${new Date(suite.timestamp).toLocaleString()}`);
    lines.push(`**User:** ${suite.identity.email}`);
    lines.push(`**Cases:** ${suite.total_cases} | **Valid:** ${suite.total_cases - m.error_cases} | **Errors:** ${m.error_cases}`);
    lines.push(`\n## System Verdict: ${v.label}\n`);
    v.details.forEach(d => lines.push(`- ${d}`));

    lines.push("\n## Core Metrics\n");
    lines.push("| Metric | Value | Target | Status |");
    lines.push("|--------|-------|--------|--------|");
    lines.push(`| Top-1 Accuracy | ${(m.top1_accuracy * 100).toFixed(1)}% | ≥45% | ${m.top1_accuracy >= 0.45 ? "✅" : "❌"} |`);
    lines.push(`| Top-3 Recall | ${(m.top3_recall * 100).toFixed(1)}% | ≥75% | ${m.top3_recall >= 0.75 ? "✅" : "❌"} |`);
    lines.push(`| Brier Score | ${m.brier_score.toFixed(4)} | ≤0.20 | ${m.brier_score <= 0.2 ? "✅" : "❌"} |`);
    lines.push(`| ECE | ${m.ece.toFixed(4)} | ≤0.08 | ${m.ece <= 0.08 ? "✅" : "❌"} |`);
    lines.push(`| Avg ΔlogP | ${m.avg_delta_logP.toFixed(3)} | ≥0.6 | ${m.avg_delta_logP >= 0.6 ? "✅" : "❌"} |`);
    lines.push(`| Avg Entropy | ${m.avg_entropy.toFixed(3)} | — | — |`);
    lines.push(`| % Ambiguous (ΔlogP<0.3) | ${m.pct_ambiguous.toFixed(0)}% | <30% | ${m.pct_ambiguous < 30 ? "✅" : "❌"} |`);
    lines.push(`| % Strong Separation (ΔlogP>1.0) | ${m.pct_strong_separation.toFixed(0)}% | — | — |`);
    lines.push(`| Overconfident (wrong+high) | ${m.overconfident_count} | 0 | ${m.overconfident_count === 0 ? "✅" : "❌"} |`);

    lines.push("\n## Per-Category Performance\n");
    lines.push("| Category | Cases | Top-1 | Top-3 | Avg Score | Avg ΔlogP | Avg Latency |");
    lines.push("|----------|-------|-------|-------|-----------|-----------|-------------|");
    m.category_metrics.forEach(cm => {
      lines.push(`| ${cm.category} | ${cm.total} | ${(cm.top1_accuracy * 100).toFixed(0)}% | ${(cm.top3_recall * 100).toFixed(0)}% | ${(cm.avg_score * 100).toFixed(1)}% | ${cm.avg_delta_logP.toFixed(2)} | ${cm.avg_latency_ms}ms |`);
    });

    if (m.diagnosis_frequency.length > 0) {
      lines.push("\n## Diagnosis Frequency (Bias Analysis)\n");
      lines.push("| Diagnosis | Count | % | Precision |");
      lines.push("|-----------|-------|---|-----------|");
      m.diagnosis_frequency.forEach(df => {
        lines.push(`| ${df.diagnosis} | ${df.count} | ${df.pct}% | ${df.correct_when_predicted}% |`);
      });
      if (m.dominant_diagnosis) {
        lines.push(`\n⚠️ **Dominant bias detected:** ${m.dominant_diagnosis}`);
      }
    }

    lines.push("\n## Case-by-Case Results\n");
    lines.push("| Case | Category | Expected | Predicted | Score | ΔlogP | T1 | T3 | ms | Error |");
    lines.push("|------|----------|----------|-----------|-------|-------|----|----|----|-------|");
    suite.results.forEach(r => {
      lines.push(`| ${r.case_id} | ${r.category} | ${r.expected_top1} | ${r.predicted_top1} | ${(r.predicted_top1_score * 100).toFixed(1)}% | ${r.delta_logP.toFixed(2)} | ${r.error ? "—" : r.top1_match ? "✅" : "❌"} | ${r.error ? "—" : r.top3_match ? "✅" : "❌"} | ${r.latency_ms} | ${r.error || "—"} |`);
    });

    lines.push("\n---\n*Report generated by DATAelixAIr V2 Evaluation Engine*");

    const blob = new Blob([lines.join("\n")], { type: "text/markdown" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `v2-evaluation-report-${Date.now()}.md`;
    a.click();
