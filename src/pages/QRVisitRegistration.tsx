...
export default function QRVisitRegistration() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Register patient (handles returning patient detection)
      const { data: regData, error: regError } = await supabase.functions.invoke("register-patient", {
        body: {
          name: name.trim(),
          age,
          gender,
      ...
      const patientId = regData.patient_id;

      // 2. Create visit with token
      const { data: visitData, error: visitError } = await supabase.functions.invoke("confirm-visit", {
        body: {
      ...
      // 3. Record triage data
      await supabase.functions.invoke("record-triage", {
        body: {
