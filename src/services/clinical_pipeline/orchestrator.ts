...
export async function runUnifiedClinicalPipeline(
  ...
  // ── Physiology vs Bayesian disagreement analysis (POST-SSAL — uses authoritative ranks) ──
  try {
    ...
    if (fusedBayesian?.diagnoses?.length) {
      ...
      console.log("=== PHYSIOLOGY vs BAYESIAN (post-SSAL) ===", mismatch.disagreement.type);
      if (typeof window !== "undefined") {
