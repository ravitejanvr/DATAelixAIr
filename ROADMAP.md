...
| 5 | Fix guideline supersession bug (add precedence field, stop serving superseded guidance, stop fabricating `year` for null `publication_date`) | Deterministic Safety | **Done 2026-09-18** — #19. Added `guideline_registry.superseded_by`, a shared `keepCurrentGuidelinesOnly()` guard used by `guideline-compliance` and `retrieve-guidelines`, and removed the hardcoded `year: 2024` fallback. Pending: confirm the migration actually applied to the live DB (not just the edge-function code deploy) and check whether any real duplicate/superseded guideline rows exist that need `superseded_by` backfilled by hand. |
