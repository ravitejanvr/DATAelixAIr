...
// ─── Implausible relationship blocklist (Safety Filter) ────────
const IMPLAUSIBLE_PAIRS: Array<[string, string]> = [
  ["stroke", "runny nose"],
  ["appendicitis", "cough"],
  ["myocardial infarction", "dandruff"],
  ["pneumonia", "itchy scalp"],
  ["meningitis", "ingrown toenail"],
  ["pulmonary embolism", "acne"],
  ["aortic dissection", "hiccups"],
  ["heart failure", "hair loss"],
  ["stroke", "sneezing"],
  ["appendicitis", "ear pain"],
];
...
function isImplausible(symptom: string, disease: string): boolean {
  ...
  return IMPLAUSIBLE_PAIRS.some(
    ([bd, bs]) => (d.includes(bd) && s.includes(bs)) || (s.includes(bd) && d.includes(bs))
  );
}
...
async function fetchPubMedAbstracts(
  ...
  for (const block of blocks) {
    ...
    const abstractText =
      block
        .match(/<AbstractText[^>]*>([\s\S]*?)<\/AbstractText>/g)
...
- evidence_level must be: "strong", "moderate", or "limited"
- diagnostic_strength must be: "definitive", "strong", "moderate", or "supportive"
- line_of_treatment must be: "first_line", "second_line", or "adjunct"
- organ_system must be one of: cardiovascular, respiratory, neurological, gastrointestinal, endocrine, renal, musculoskeletal, dermatological, hematological, immunological, ent, psychiatric`,
        },
        {
          role: "user",
              ...
              properties: {
                ...
                physiology_diagnosis: {
                  ...
                  items: {
                    ...
                    required: ["physiology_process", "disease_name", "confidence_score"],
                    additionalProperties: false,
                  },
                },
                disease_tests: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      diagnostic_strength: {
                        type: "string",
                        enum: ["definitive", "strong", "moderate", "supportive"],
                      },
                    },
                ...
                disease_treatments: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      line_of_treatment: {
                        type: "string",
                        enum: ["first_line", "second_line", "adjunct"],
                      },
...
async function insertRelationships(
  ...
  const inserted: Record<string, number> = {
    symptom_likelihoods: 0,
    symptom_physiology_map: 0,
    physiology_diagnosis_map: 0,
    disease_tests: 0,
    disease_treatments: 0,
  };
  let rejected = 0;

  // 1. Symptom likelihoods
  for (const sl of extracted.symptom_likelihoods) {
    if (sl.probability < 0.3) { rejected++; continue; }
    if (isImplausible(sl.symptom, sl.disease_name)) { rejected++; continue; }

    const { data: existing } = await supabase
      .from("symptom_likelihoods")
      .select("id")
      .eq("symptom", sl.symptom.toLowerCase())
      .eq("disease_name", sl.disease_name.toLowerCase())
      .maybeSingle();
    if (existing) continue;

    const { error } = await supabase.from("symptom_likelihoods").insert({
      symptom: sl.symptom.toLowerCase(),
      disease_name: sl.disease_name.toLowerCase(),
      probability: sl.probability,
      weight: sl.weight,
      evidence_level: sl.evidence_level,
    });
    if (!error) inserted.symptom_likelihoods++;
  }

  // 2. Symptom physiology
  for (const sp of extracted.symptom_physiology) {
    const { data: existing } = await supabase
      .from("symptom_physiology_map")
      .select("id")
      .eq("symptom", sp.symptom.toLowerCase())
      .eq("physiology_process", sp.physiology_process.toLowerCase())
      .maybeSingle();
    if (existing) continue;

    const { error } = await supabase.from("symptom_physiology_map").insert({
      symptom: sp.symptom.toLowerCase(),
      physiology_process: sp.physiology_process.toLowerCase(),
      organ_system: sp.organ_system.toLowerCase(),
    });
    if (!error) inserted.symptom_physiology_map++;
  }

  // 3. Physiology → diagnosis
  for (const pd of extracted.physiology_diagnosis) {
    const { data: existing } = await supabase
      .from("physiology_diagnosis_map")
      .select("id")
      .eq("physiology_process", pd.physiology_process.toLowerCase())
      .eq("disease_name", pd.disease_name.toLowerCase())
      .maybeSingle();
    if (existing) continue;

    const { error } = await supabase.from("physiology_diagnosis_map").insert({
      physiology_process: pd.physiology_process.toLowerCase(),
      disease_name: pd.disease_name.toLowerCase(),
  ...
  for (const dt of extracted.disease_tests) {
    const { data: existing } = await supabase
      .from("disease_tests")
      .select("id")
      .eq("disease_name", dt.disease_name.toLowerCase())
      .eq("test_name", dt.test_name.toLowerCase())
      .maybeSingle();
    if (existing) continue;
  ...
  for (const tr of extracted.disease_treatments) {
    const { data: existing } = await supabase
      ...
      .select("id")
      .eq("disease_name", tr.disease_name.toLowerCase())
      .eq("drug_name", tr.drug_name.toLowerCase())
      .maybeSingle();
    if (existing) continue;
...
serve(async (req) => {
  ...
  try {
    ...
    for (const disease of diseases.slice(0, 10)) {
      ...
      pipelineStats.push({
        stage: `retrieval:${disease}`,
        count: allAbstracts.length,
        duration_ms: Date.now() - s1Start,
      });

      if (allAbstracts.length === 0) {
        console.warn(`[KG-Extract] No abstracts found for: ${disease}`);
        allResults[disease] = { skipped: true, reason: "no_abstracts" };
        continue;
      }

      // Stage 2+3: AI extraction (entity recognition + relationship extraction)
      const s2Start = Date.now();
      let extracted: ExtractionResult;
      try {
        extracted = await extractRelationships(disease, allAbstracts, lovableKey);
      } catch (e) {
        console.error(`[KG-Extract] AI extraction failed for ${disease}:`, e);
        allResults[disease] = { skipped: true, reason: "extraction_error", error: e.message };
        continue;
      }
      pipelineStats.push({
        stage: `extraction:${disease}`,
        count:
          extracted.symptom_likelihoods.length +
          extracted.symptom_physiology.length +
          extracted.physiology_diagnosis.length +
          extracted.disease_tests.length +
          extracted.disease_treatments.length,
      ...
      if (!dryRun) {
        ...
        const { inserted, rejected } = await insertRelationships(
          supabase,
          extracted,
          disease,
          "pubmed_extraction"
        ...
        allResults[disease] = { inserted, rejected, total_extracted: extracted };
      } else {
        allResults[disease] = {
          dry_run: true,
          extracted: {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        diseases_processed: Object.keys(allResults).length,
        results: allResults,
